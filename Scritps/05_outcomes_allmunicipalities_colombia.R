###################################
# DID estimates all municipalities
###################################
...
# health_lack 
m1 <- felm(d$outcome_health_lack ~ d$min_distance + d$post + d$min_distance*d$post + d$edu + d$age + d$female + d$num_victims + d$size | 0 | 0 | d$municipio, keepX=TRUE)
summary(m1)
pe1 = m1$beta[9]*100
...
# education_lack
m2 <- felm(d$outcome_lack_education ~ d$min_distance + d$post + d$min_distance*d$post + d$edu + d$age + d$female + d$num_victims + d$size | 0 | 0 | d$municipio, keepX=TRUE)
summary(m2)
pe2 = m2$beta[9]*100
...
cairo_pdf(file="~/Dropbox/Migration Colombia/03_manuscript/figures/figA1.pdf", 
          width=10, 
          height=5)
...
cairo_ps(file="~/Dropbox/Migration Colombia/03_manuscript/figures/figA1.eps", 
          width=10,
          height=5)
ggplot(results_per, aes(x=outcome_per, y=pe_per)) + geom_hline(yintercept=0, size=1, color = "gray", linetype = 1) + coord_flip(ylim = c(-.025, .025)) + geom_pointrange(aes(ymin=lower95_per,ymax=upper95_per),size=1,position = position_dodge(0.9)) + theme(legend.position="none") + ylab("Change in concerns (in percentage points)") + xlab("") + theme_grey(base_size = 20) 
dev.off()
